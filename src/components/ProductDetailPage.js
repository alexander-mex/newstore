...
const ProductPage = ({ darkMode, language, filters: initialFilters }) => {
  ...
  const filterOptionsController = useRef(null);
  ...
  const fetchFilterOptions = useCallback(async () => {
    ...
    if (filterOptionsController.current) {
      filterOptionsController.current.abort();
    }

    // Create new AbortController
    filterOptionsController.current = new AbortController();

    try {
      const response = await axios.get("/api/products/filters", {
        params: { category: filters.category },
        signal: filterOptionsController.current.signal,
      ...
      if (error.name === 'CanceledError' || error.code === 'ECONNABORTED') {
        console.log("Filter options request was cancelled or timed out");
        return;
      }
      ...
      try {
        ...
        if (searchQuery) {
          ...
          const response = await axios.get("/api/products/search", {
            ...
            signal: productsController.current.signal,
        ...
        if (error.name === 'CanceledError' || error.code === 'ECONNABORTED') {
          console.log("Products request was cancelled or timed out");
          return;
        }
  ...
  const loadMoreProducts = async () => {
    ...
    try {
      ...
      if (error.name === 'CanceledError' || error.code === 'ECONNABORTED') {
        console.log("Load more request was cancelled or timed out");
        return;
      }
